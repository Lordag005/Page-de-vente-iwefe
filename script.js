...
function updateCartDisplay() {
    ...
    cartItems.forEach(item => {
      ...
      const price = parseFloat(item.price.replace('$', '')); // Convertir le prix en nombre
      totalAmount += price; // Ajouter le prix au total
    });
  
    const totalDisplay = document.querySelector('.total-price');
    totalDisplay.textContent = `$${totalAmount.toFixed(2)}`; // Afficher le total avec 2 décimales
  }
